...
function(x, levels, locations=NULL, plot=TRUE){
    ...
    for(i in levels){
        ...
        probability = c(probability, optimize(contour.z, c(0, max(x$probability)), tol=.Machine$double.eps)$minimum)
    }
   ans = vector("list", 2)
   ...
   if(plot){
        ...
        z = matrix(temp$z, nrow=length(unique(temp$x)), ncol=length(unique(temp$y)), byrow=TRUE)
        contour(x=unique(temp$x), y=unique(temp$y), z, levels=ans[[2]], 
                drawlabels=TRUE, labels=ans[[1]], xlab="X", ylab="Y")
        points(locations[,1], locations[,2], pch=19, cex=0.5, col="red")
